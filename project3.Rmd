...
## Intuition and Explanation on significant factors
...
absent$timecat <- ifelse(absent$timecat == 0, "not.absent", ifelse (absent$timecat<=56, "one.week", "more.than.one.week"))
...
train<-cbind(absam[,-44],catint)


n<-names(train)
f<-f <- as.formula(paste("not.absent + one.week + more.than.one.week ~", paste(n[!n %in% c("not.absent", "one.week", "more.than.one.week" )], collapse = " + ")))
f
...
original_values <- max.col(train[, 44:49]) #takes column number with the maximum value
...
library(C50)
abs_model <- C5.0(abs_train_tree[-15], abs_train_tree$timecat)
...
abs_boost10 <- C5.0(abs_train_tree[-15], abs_train_tree$timecat,
                       trials = 10)
abs_boost10
summary(abs_boost10)
plot(abs_boost10)
...
abs_boost100 <- C5.0(abs_train_tree[-15], abs_train_tree$timecat,
                        trials = 100)
...
error_cost <- matrix(c(0, 1, 4, 0,1,4,0,1,4), nrow = 3)
error_cost
...
abs_cost <- C5.0(abs_train_tree[-15], abs_train_tree$timecat,
                          costs = error_cost)
...
train_absent <- abs_rand[tindex,]   # Create training set
test_absent <- abs_rand[-tindex,]   # Create test set
## Construct SVM Model
library(e1071)
...
```{r}
setwd("/Users/candicelin/Desktop/TO 414/Absenteeism_at_work_AAA")
absentANN <-read.csv("Absenteeism_at_work.csv")
str(absentANN)
...
```
With 15 neurons, we get a pretty high accruacy, ~85%.
