...
Now, let's run linear models again and see if they give us better R^2.
...
# Classification
Here are the models we made in order to find out the best model in predicting roughly how many days/weeks an employee will be absent.
##1. KNN Model
```{r}
...
Decision Tree model gives a really high accuracy (~90%) and a high Kappa (~83%). Let's see if boosting can further improve the accuracy and Kappa.
...
Boosted decision tree with 10 trials increased the accuracy to 91% and Kappa to 85%.
...
Linear KSVM model gives 77% accuracy and 59% Kappa;KSVM model with rbf kernal gives 95% accuracy and 91% Kappa.
