...
absent_mm <- as.data.frame(model.matrix(~ . -1, absent_r[-14])) ##Breaking factors into dummy variables
...
abs_train <- absent_SP[1:728, -43]
abs_test <- absent_SP[729:828,-43]
abs_train_label <- absent_SP[1:728, 43]
abs_test_label <- absent_SP[729:828,43]

##building KNN model
KNN_MODEL <- knn(train = abs_train, test = abs_test,
                      cl = abs_train_label, k=10)
...
error_cost <- matrix(c(0, 5, 5, 5,0,5,5,5,0), nrow = 3)
error_cost
...
abs_cost <- C5.0(abs_train_tree[-15
                                ], abs_train_tree$timecat,
                          costs = error_cost)
