...
We struggled a lot with creating categories for Absenteeism in hours. In the beginning, we decided that predicting how many hours an employee will be absent will not be more straightforward or add more business values than predicting how many days/weeks an employee will be absent.
So first, we created 6 categories("not.absent","less.than.one.day","one.to.four.days","four.days.to.one.week","one.to.two.weeks","more.than.two.weeks"),but because there are too many categories we were unable to make models with satisfying Accuracy or Kappa.Also, since the dataset is skewed (the majority concentrated in less.than.one.day", we got a lots of test data sets with no data points in "one.to.two.weeks","more.than.two.weeks".After some trials, we decided to narrow down to three categories,so our second try is "less.than.day","less.than.one.week","more.than.one.week".
...
+ Diseases of the circulatory system go down as distance from work increases and also with increase in hitting the target. So if you are performing well and live farther away from your job, you are more less likely to have issues with circulatory system
